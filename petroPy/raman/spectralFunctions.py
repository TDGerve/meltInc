...
def GaussLorentz(x, amplitude, center, width, baselevel, shape):
    ...
    return gaussian(x, amplitude*(1-shape), center, width, baselevel) + lorentzian(x, amplitude*shape, center, width, baselevel)
...
def composeCurves(x, centers, amplitudes, widths, shapes, baselevel= 0):
    ...
    params = [{'center': i, 'amplitude': j, 'width': k, 'baselevel': l, 'shape': m} for _, (i, j, k, l, m) in enumerate(zip(centers, amplitudes, widths, baselevels, shapes))]
...
def smooth(y, smoothType= 'gaussian', kernelWidth= 9):
    """smoothtype: movingAverage or gaussian
        kernelWidth (int): width of smoothing kernel in elements of y
    """
    ...
    if smoothType == 'movingAverage':
        ...
        kernel= np.fromiter((gaussian(x, 1, 0, kernelWidth/3, 0) for x in range(-(kernelWidth-1)//2, (kernelWidth+1)//2, 1 )), np.float)
...
def long_correction(x, intensities, T_C= 25., laser= 532.18, normalisation= 'area'):
...
def diads(x, intensities, peak_prominence= 40, fit_window= 8, curve= 'GL'): 
    ...
    amplitudes= intensities[signal.find_peaks(intensities, prominence= peak_prominence)[0]]
    ...
    widths= (signal.peak_widths(intensities, signal.find_peaks(intensities, prominence= peak_prominence)[0])[0]*abs(np.diff(x).mean()))[sort_index] #full width half maximum in wavenumbers
    ...
    #trim fit areas
    trim1= (x > (init_values1[1] - init_values1[2]*fit_window)) & (x < (init_values1[1] + init_values1[2]*fit_window))
    trim2= (x > (init_values2[1] - init_values2[2]*fit_window)) & (x < (init_values2[1] + init_values2[2]*fit_window))
    x1= x[trim1]
    x2= x[trim2]
    ...
    fit_params1= least_squares(fun= residuals, x0= init_values1, bounds= bounds, args= (x1, intensity1)).x
    fit_params2= least_squares(fun= residuals, x0= init_values2, bounds= bounds, args= (x2, intensity2)).x

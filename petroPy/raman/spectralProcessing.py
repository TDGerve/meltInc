from . import spectralFunctions as sf
import numpy as np
from warnings import warn
# from rampy import baseline as blcorrect
...
class H2O(RamanProcessing):
	...
	# # remove normalise and norm as usable attributes
	# def __getattribute__(self, name ):

	# 	if name in ['normalise', 'norm']: raise AttributeError(name)
	# 	else: return super(H2O, self ).__getattribute__(name)	

	# # remove normalise and norm from dir call
	# def __dir__(self):

	# 	return sorted((set(dir(self.__class__ )) | set(self.__dict__.keys())) - set(['normalise', 'norm']))

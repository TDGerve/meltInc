...
def Kd_ToplisIter(liquid, olivine_Fo, T_K, P, H2O=None):
    ...
    SiO2mol_A = SiO2_A_toplis(
        melt_molar["SiO2"], melt_molar["Na2O"], melt_molar["K2O"], Phi, H2O
    )
    ...
    while sum(forsterite_delta > 0.01) > 1:
        ...
        melts.loc[iterate, "Kd"] = Kd_toplis(
            T_K, P, forsterite_EQ.loc[iterate], SiO2mol_A.loc[iterate]
        )
        forsterite.loc[iterate] = forsterite_EQ.loc[iterate].copy()
        forsterite_EQ.loc[iterate] = 1 / (
            1
            + ox_fact
            * melts.loc[iterate, "Kd"]
            * (melts.loc[iterate, "FeO"] / melts.loc[iterate, "MgO"])
        )

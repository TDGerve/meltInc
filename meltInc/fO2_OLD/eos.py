...
def teosth(phase, pkbar, t, tref= 298.15, **kwargs):
    ...
    intVdP = pkbar * v0 * (1 - a + a * (np.sign((1 - b * pth)) * math.pow(abs(1 - b * pth), (1-c)) - np.sign((1 + b * (pkbar - pth))) * math.pow(abs((1 + b * (pkbar - pth))), (1- c ))) / (b * (c - 1) * pkbar))
...
def landauhppressure(phase, pkbar, t):
    ...
    smax, vmax, tc0 = [getattr(EOSparams, phase)[i]
                       for i in ['smax', 'vmax', 'Tc0']]
...
def CpHoll(phase, t, tref=298.15):
    ...
    a, b, c, d = [getattr(EOSparams, phase)[i]
                  for i in ['cp_a', 'cp_b', 'cp_c', 'cp_d']]
